    ...
    passport.use(
        ...
        new LocalStrategy(
            ...
            function(req, email, password, done) {
                ...
                if (email != sanitizer.sanitize(email) || password != sanitizer.sanitize(password)
                    || req.body.username != sanitizer.sanitize(req.body.username)){
                    return done(null, false, req.flash("newapplyMessage", "Invalid input"));
                }
                
                if (password != req.body.confirmPassword){
                    return done(null, false, req.flash('newapplyMessage', "Passwords do not match!"));
                }
                else{
                    // asynchronous
                    // User.findOne wont fire unless data is sent back
                    process.nextTick(function() {
                        ...
                        //See how many users there are in the database to determine if this is the first user
                        Apply.count({}, function(err, count){
                            //The first user is admin
                            var userType="user";
                            if (count == 0){
                                userType="admin"
                            ...
                            // find a user whose email is the same as the forms email
                            Apply.findOne({ 'local.email' :  email }, function(err, user) {
                                // if there are any errors, return the error
                                if (err)
                                    return done(err);
                                ...
                                // check to see if theres already a user with that email
                                if (user) {
                                    return done(null, false, req.flash('newapplyMessage', 'That email is already taken.'));
                                } else {

                                    // if there is no user with that email
                                    // create the user
                                    var newapply = new Apply();
                                    newapply.accountType="local";
                                    newapply.userType=userType;
                                    newapply.imgPath = "/avatar/default.png";

                                    //default field
                                    newapply.description = "no description";
                                    //newUser.phone = "N/A";
                                    // set the user's local credentials
                                    newapply.local.username= req.body.username;
                                    newapply.local.email    = email;
                                    newapply.city    = req.body.city;
                                    //newapply.local.password = password;
                                    newapply.latitude = req.body.latitude;
                                    newapply.altitude = req.body.altitude;       
                                    newapply.local.password = newapply.generateHash(password);

                                    // save the user
                                    newapply.save(function(err) {
                                        if (err)
                                            throw err;
                                        return done(null, newapply);
                                    });
                                }
